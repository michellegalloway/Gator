...
bool GatorDataLoader::LoadLines(string cfgfilename)
{
	...
	cout << "\nNumber of loaded lines: " << mv_lines->size() << endl;
...
bool GatorDataLoader::SetSampleCalib(string calibdir)
{
	...
	cout << "\nOpening the calibration file for the sample data <" << m_datacalibdir+string("calibration.root") << ">" << endl;
	...
	if(!mp_data_calib_bw){
		cerr << "\nGatorDataLoader::SetSampleCalib --> WARNING: bad pointer for the function <calib_fcn_bw>: " << mp_data_calib_bw << endl;
	}
	mp_data_calib_fw = (TF1*)calibfile -> Get("calib_fcn_fw");
	if(!mp_data_calib_fw){
		cerr << "\nGatorDataLoader::SetSampleCalib --> WARNING: bad pointer for the function <calib_fcn_fw>: " << mp_data_calib_fw << endl;
	}
	calibfile->Close();
	
	cout << "\nOpening the resolution file for the sample data <" << m_datacalibdir+string("resolution.root") << ">" << endl;
	TFile *resolfile = new TFile((m_datacalibdir+string("calibration.root")).c_str(), "read");
	mp_data_MCA_resol = (TF1*)resolfile -> Get("resol_ADC_func");
	if(!mp_data_MCA_resol){
		cerr << "\nGatorDataLoader::SetSampleCalib --> WARNING: bad pointer for the function <resol_ADC_func>: " << mp_data_MCA_resol << endl;
	}
	resolfile->Close();
...
bool GatorDataLoader::SetBackgroundCalib(string calibdir)
{
	...
	cout << "\nOpening the calibration file for the background data <" << m_Bgcalibdir+string("calibration.root") << ">" << endl;
	TFile *calibfile = new TFile((m_Bgcalibdir+string("calibration.root")).c_str(), "read");
	mp_Bgcalib_bw = (TF1*)calibfile -> Get("calib_fcn_bw");
	if(!mp_Bgcalib_bw){
		cerr << "\nGatorDataLoader::SetBackgroundCalib --> WARNING: bad pointer for the function <calib_fcn_bw>: " << mp_Bgcalib_bw << endl;
	}
	mp_Bgcalib_fw = (TF1*)calibfile -> Get("calib_fcn_fw");
	if(!mp_Bgcalib_fw){
		cerr << "\nGatorDataLoader::SetBackgroundCalib --> WARNING: bad pointer for the function <calib_fcn_fw>: " << mp_Bgcalib_fw << endl;
	}
	calibfile->Close();
	
	cout << "\nOpening the resolution file for the background data <" << m_Bgcalibdir+string("resolution.root") << ">" << endl;
	TFile *resolfile = new TFile((m_Bgcalibdir+string("resolution.root")).c_str(), "read");
	mp_BgMCA_resol = (TF1*)resolfile -> Get("resol_ADC_func");
	if(!mp_BgMCA_resol){
		cerr << "\nGatorDataLoader::SetBackgroundCalib --> WARNING: bad pointer for the function <resol_ADC_func>: " << mp_BgMCA_resol << endl;
	}
	resolfile->Close();
