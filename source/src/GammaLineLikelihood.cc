...
void GammaLineLikelihood::SetInitPars(vector<double> parVal)
{
	...
	LikelihoodClass::SetInitPars(initpar);
	return;
}
...
double GammaLineLikelihood::MinLogProb(const double* parVal)
{
	...
	GammaLineLikelihood* this_ptr = static_cast<GammaLineLikelihood*>(LikelihoodClassHolder::instance());
...
void GammaLineLikelihood::MetropolisMLE()
{
	...
	GammaLineLikelihood::MetropolisMLE(NULL);
	fMinimized = true;
	return;
}
...
void GammaLineLikelihood::Minuit2MLE(vector<double> &pars)
{
	...
	fMinFunctor = new ROOT::Math::Functor(&GammaLineLikelihood::MinLogProb, fParsN);
