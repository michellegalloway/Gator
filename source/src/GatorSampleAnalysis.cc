...
void GatorSampleAnalysis::doAnalysis()
{
	...
	cout << "\nLogging file <" << m_logfilename << ">" << endl << endl;
	
	ofstream logfile(m_logfilename.c_str());
	
	logfile << "Number of lines to be analized: " << nLines << endl << endl;
	...
	for(int iLine=0; iLine<nLines; iLine++){
		...
		if(mb_BGflag){
			...
			*/
			...
			if( (line->BgLdCnts<line->BgLineCnts) || mb_BgForced ){
		...
		if(line->BgDetected){
			double bgcnts = (md_aqtime/md_Bgtime)*max(0.,line->BgLineCnts);
			double bgcnts_err = (md_aqtime/md_Bgtime)*((max(0.,line->BgLineCnts)/line->BgLineCnts)*line->BgLineCnts_err);
		...
		if(mb_BGflag){
			...
			logfile << " Calibration dir: <" << m_Bgcalibdir << ">" << endl;
			logfile << " MCA center = " << line->BgMCAcenter << endl;
			logfile << " MCA sigma = " << line->BgMCAsigma << endl;
			logfile << " Left compton region = [" << mp_Bgcalib_fw->Eval(line->BgLowEdgeL) << "," << mp_Bgcalib_fw->Eval(line->BgUpEdgeL) << "] keV --> (" << line->BgLowEdgeL << "," << line->BgUpEdgeL << ") = " << line->BgbinL << " chs" << endl;
			logfile << " Left compton counts = " << line->BgcountsL << endl;
			logfile << " Signal region = [" << mp_Bgcalib_fw->Eval(line->BgLowEdgeS) << "," << mp_Bgcalib_fw->Eval(line->BgUpEdgeS) << "] keV --> (" << line->BgLowEdgeS << "," << line->BgUpEdgeS << ") = " << line->BgbinS << "chs" << endl;
			logfile << " Signal region counts = " << line->BgcountsS << endl;
			logfile << " Right compton region = [" << mp_Bgcalib_fw->Eval(line->BgLowEdgeR) << "," << mp_Bgcalib_fw->Eval(line->BgUpEdgeR) << "] keV --> (" << line->BgLowEdgeR << "," << line->BgUpEdgeR << ") --> " << line->BgbinR << " chs" << endl;
		...
		logfile << "Sample:" << endl;
		logfile << " Calibration dir: <" << m_datacalibdir << ">" << endl;
		logfile << " MCA center = " << line->MCAcenter << endl;
		logfile << " MCA sigma = " << line->MCAsigma << endl;
		logfile << " Left compton region = [" << mp_data_calib_fw->Eval(line->LowEdgeL) << "," << mp_data_calib_fw->Eval(line->UpEdgeL) << "] keV --> (" << line->LowEdgeL << "," << line->UpEdgeL << ") = " << line->binL << "chs" << endl;
		logfile << " Left compton counts = " << line->countsL << endl;
		logfile << " Signal region = [" << mp_data_calib_fw->Eval(line->LowEdgeS) << "," << mp_data_calib_fw->Eval(line->UpEdgeS) << "] keV --> (" << line->LowEdgeS << "," << line->UpEdgeS << ") = " << line->binS << "chs" << endl;
		logfile << " Signal region counts = " << line->countsS << endl;
		logfile << " Right compton region = [" << mp_data_calib_fw->Eval(line->LowEdgeR) << "," << mp_data_calib_fw->Eval(line->UpEdgeR) << "] keV --> (" << line->LowEdgeR << "," << line->UpEdgeR << ") --> " << line->binR << " chs" << endl;
...
void GatorSampleAnalysis::WriteEffTable(string txtfilename)
{
	...
	cout << "\nEfficiencies written in: <" << txtfilename << ">" << endl << endl;
