...
void GatorCounter::ResetCounts(){
	
	unsigned nLines = mv_lines->size();
	
	for(unsigned iLine=0; iLine<nLines; iLine++){
		
		(mv_lines->at(iLine)).countsL=0;
		(mv_lines->at(iLine)).countsS=0;
		(mv_lines->at(iLine)).countsR=0;
		(mv_lines->at(iLine)).BgcountsL=0;
		(mv_lines->at(iLine)).BgcountsS=0;
		(mv_lines->at(iLine)).BgcountsR=0;
		(mv_lines->at(iLine)).PeakCnts=0;
		(mv_lines->at(iLine)).PeakCnts_err=0;
		(mv_lines->at(iLine)).CompCnts=0;
		(mv_lines->at(iLine)).CompCnts_err=0;
		(mv_lines->at(iLine)).LineCnts=0;
		(mv_lines->at(iLine)).LineCnts_err=0;
		(mv_lines->at(iLine)).LdCnts=0;
	}
