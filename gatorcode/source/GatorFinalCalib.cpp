...
*/
	...
	c1 -> cd(2);
	gr_cal_res_fw -> GetXaxis() -> SetTitle("[keV]");
	...
	c2 -> cd(2);
	gr_cal_res_bw -> GetXaxis() -> SetTitle("MCA channels");
	gr_cal_res_bw -> GetYaxis() -> SetTitle("MCA channels");
	...
	if(ans==string("y")  || ans==string("Y")){
		...
		resolFile -> WriteTObject(resol_ADC_func,0,"Overwrite");
